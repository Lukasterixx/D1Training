...
| --- | --- | --- |
| Interaction point | Link7_1 pincer tip: end-face centre `(0.0547, 0.0060, 0.0170)` m in Link7_1 coordinates (`tool_point.py`); `--tip_body`, `--tip_offset` | CAD-derived, not measured on the arm; matches the model to 0.9 µm in simulation. Grasping would need the point between the pincers, and **the cost of not using it is now measured**: `Joint7_1` slides this point up to 30 mm as the jaws open, and UniFP's policy — which drops both jaws from its observation — tracks 0.91 cm from a goal with the jaws shut and 3.50 cm fully open (F-094). Putting the measured travel back into the commanded goal recovers it. `--tip_body Link6 --tip_offset 0 0 0` is the pre-15-September point |
...
| Latency | Legs 0–2 physics steps (estimated); D1 commands 10 Hz (estimated) and feedback 9 Hz (measured, F-020), random phase | Measure Go2 command-to-motion delay. `--latency none` is a declared sensitivity run |
| D1 firmware motion | `--arm_trajectory measured`: 10 ms dead time, trapezoid at 15.5/17.4 rad/s², restart from rest on each new setpoint, velocity feedforward to the drive (F-045, F-046) | Fitted to one unloaded arm in one posture per joint; the 5° streaming cap misfits (4.3 vs 2.5°/cycle); whether an *identical* re-sent setpoint restarts the plan is untested on hardware. A streamed arm moves at ~0.8 rad/s, not 1.27 |
| D1 limits | Published torques; measured speed ceilings 1.25/1.29/1.23/1.21/1.25/1.25 rad/s (F-033, lower bounds) | Loaded and posture-dependent speed unmeasured. PhysX applies exactly these values (`verify`) |
